...
const resetFilters = () => {
    ...
    router.get(route('admin.aplicationsA.acceptedApplications'), {}, {
        preserveState: true,
        replace: true,
    });
};
...
const saveNote = () => {
    if (isEditing.value && currentNote.value) {
        ...
        formNote.put(route('admin.aplicationsA.update-note', currentNote.value.id), {
            preserveScroll: true,
            onSuccess: () => closeNoteModal(),
        });
    } else {
        // Dodawanie nowej notatki
        formNote.post(route('admin.aplicationsA.store-note', currentApplication.value.id), {
            preserveScroll: true,
            onSuccess: () => closeNoteModal(),
        });
    }
...
watch(form, debounce(function (value) {
    ...
    router.get(route('admin.aplicationsA.acceptedApplications'), rest, {
        preserveState: true,
        preserveScroll: true,
        replace: true
    });
                ...
                <div class="bg-white shadow-xl sm:rounded-lg p-6">
                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                        ...
                        <div class="flex space-x-2">
                            <Link v-if="otherCount > 0" :href="route('admin.aplicationsA.index')"
                                  class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('translate.aplications') }}
                            </Link>
                            <Link v-if="maybeCount > 0" :href="route('admin.aplicationsA.maybeApplications')"
                            ...
                            <Link v-if="noCount > 0" :href="route('admin.aplicationsA.noApplications')"
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr v-for="application in applications.data" :key="application.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <Link :href="route('admin.aplicationsA.show', application.id)"
