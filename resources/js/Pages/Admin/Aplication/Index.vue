...
const exportToCSV = () => {
    ...
    // Symulacja opóźnienia, która może być zastąpiona rzeczywistym pobieraniem
    setTimeout(() => {
        window.location.href = route('admin.aplicationsA.export', ['form', form.value]);

        // Resetujemy stan po krótkim czasie, aby użytkownik zobaczył, że akcja została ukończona
        setTimeout(() => {
...
<!--                            DRIVE-->
                        ...
                        <div class="flex space-x-2">
                            <Link v-if="acceptedCount > 0" :href="route('admin.aplicationsA.acceptedApplications')" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:bg-green-700 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('translate.statusPotentialUsers') }}
                            </Link>
                            <Link v-if="maybeCount > 0" :href="route('admin.aplicationsA.maybeApplications')" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 focus:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('translate.New') }}
                            </Link>
                            <Link v-if="noCount > 0" :href="route('admin.aplicationsA.noApplications')" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-700 active:bg-red-800 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('translate.No') }}
                            </Link>
                            <button
                                @click="exportToCSV"
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tr v-for="application in applications.data" :key="application.id">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <Link :href="route('admin.aplicationsA.show', application.id)" class="text-indigo-600 hover:text-indigo-900 mr-4">{{ __('translate.applicationDetails') }}</Link>
                                </td>
                            </tr>
                            </tbody>
                        </table>
