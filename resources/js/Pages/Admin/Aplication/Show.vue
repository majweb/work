    ...
    <AppLayout :title="__('translate.aplicationDetails')">
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link
                    :href="route('admin.aplicationsA.index')"
                    ...
                    <div class="p-8 md:p-10">
                        ...
                        <div class="mt-12 grid grid-cols-1 gap-10 lg:grid-cols-3">
                            ...
                            <div class="lg:col-span-2 space-y-8">
                                ...
                                <!-- PYTANIA KANDYDATA (Podgląd dla Admina) -->
                                <div v-if="application.candidate" class="rounded-[2.5rem] border border-gray-100 bg-white p-8 shadow-sm">
                                    ...
                                    <div v-if="application.candidate.answers && application.candidate.answers.length > 0" class="space-y-6">
                                        <div v-for="(answer, index) in application.candidate.answers" :key="answer.id" class="rounded-2xl bg-gray-50/50 p-6 border border-gray-100/50">
                                            <div class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">
                                                {{ index + 1 }}. {{ answer.question?.title[usePage().props.language] }}
                                            </div>

                                            <div v-if="answer.text_answer" class="mt-2">
                                                <div class="block w-full rounded-xl border border-gray-100 bg-white p-4 text-sm shadow-inner text-gray-700">
                                                    {{ answer.text_answer }}
                                                </div>
                                            </div>

                                            <div v-else-if="answer.boolean_answer !== null" class="mt-2">
                                                <span class="rounded-xl px-6 py-2 text-[10px] font-black uppercase tracking-widest text-white"
                                                      :class="answer.boolean_answer ? 'bg-emerald-600' : 'bg-red-600'">
                                                    {{ answer.boolean_answer ? __('translate.yes') : __('translate.no') }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    <div v-else class="text-center py-8">
                                        <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">{{ __('translate.noAnswers') }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="rounded-[2.5rem] border border-gray-100 bg-white p-8 self-start space-y-8 shadow-sm">
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-col gap-3">
                                        ...
                                        <Link
                                            v-if="application.aplication_user_id"
                                            :href="route('admin.workers.edit', application.aplication_user_id)"
