<template>
    <AppLayout title="Akceptacja banerów">
        <div class="p-8 lg:p-12 space-y-12">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        Zarządzanie banerami reklamowymi
                    </p>
                </div>
            ...
            <div class="rounded-[3rem] bg-white shadow-xl shadow-blue-900/5 border border-gray-100 overflow-hidden">
                <div class="p-8">
                        ...
                        <table class="w-full text-left border-separate border-spacing-y-3">
                            <thead>
                                <tr class="text-[#0A2C5C]/30 text-[9px] font-black uppercase tracking-widest">
                                    ...
                                    <th class="px-6 py-2" @click="sort('created_at')">Data dodania</th>
                            ...
                            <tbody>
                                <tr v-if="props.banners.data.length === 0">
                                    <td colspan="5" class="text-center py-16">
                                        <p class="text-xs font-black text-gray-300 uppercase tracking-widest">Nie znaleziono żadnych banerów</p>
                                    </td>
                                </tr>
                                <tr v-else v-for="banner in props.banners.data" :key="banner.id" class="group">
                                    <td class="px-6 py-4 bg-gray-50/50 rounded-l-2xl border-y border-l border-gray-100 group-hover:bg-white transition-all">
                                        <span class="text-xs font-black text-[#0A2C5C]">#{{ banner.id }}</span>
                                    </td>
                                    <td class="px-6 py-4 bg-gray-50/50 border-y border-gray-100 group-hover:bg-white transition-all">
                                        <div class="flex items-center gap-2">
                                            <img v-if="banner.user?.profile_photo_url" :src="banner.user.profile_photo_url" class="w-6 h-6 rounded-full object-cover shadow-sm">
                                            ...
                                            <span class="text-xs font-black text-[#0A2C5C] uppercase tracking-tight">{{ banner.user?.name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 bg-gray-50/50 border-y border-gray-100 group-hover:bg-white transition-all">
                                        <span class="text-[10px] font-bold text-gray-500">{{ new Date(banner.created_at).toLocaleDateString() }}</span>
                                    </td>
                                    <td class="px-6 py-4 bg-gray-50/50 border-y border-gray-100 group-hover:bg-white transition-all">
                                        <span v-if="banner.active" class="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-green-100 text-[8px] font-black text-green-700 uppercase tracking-widest">
                                            AKTYWNY
                                        </span>
                                    ...
                                    <td class="px-6 py-4 bg-gray-50/50 border-y border-gray-100 group-hover:bg-white transition-all">
                                        <span v-if="banner.active_admin" class="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-blue-100 text-[8px] font-black text-blue-700 uppercase tracking-widest">
                                            ZAAKCEPTOWANY
                                        </span>
                                    ...
                                    <td class="px-6 py-4 bg-gray-50/50 rounded-r-2xl border-y border-r border-gray-100 group-hover:bg-white transition-all text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <Link :href="route('admin.banners-accepts.show', banner)" class="p-2 rounded-lg bg-white border border-gray-100 text-[#0A2C5C] hover:bg-[#0A2C5C] hover:text-white transition-all">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                                </svg>
                                            </Link>
                                            <button @click="openModal(banner)" class="p-2 rounded-lg bg-red-50 text-red-500 hover:bg-red-500 hover:text-white transition-all">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
                    ...
                    <Pagination v-if="banners.total > banners.per_page" class="mt-8" :links="banners.links" />
                </div>
            </div>
            ...
            <DialogModal :show="confirmDelete" @close="confirmDelete = false">
                ...
                <template #footer>
                    <div class="flex gap-4">
                        ...
                        <DangerButton @click="deleteBanner" class="!rounded-xl !py-3 !px-8 !font-black !uppercase !tracking-widest !text-[10px] !shadow-xl !shadow-red-500/20">
                            Usuń
                        </DangerButton>
                    </div>
                </template>
...
const deleteBanner = () => {
    if (selectedBanner.value) {
        router.delete(route('admin.banners-accepts.destroy', selectedBanner.value), {
            onSuccess: () => {
...
watch(params.value, debounce(function (value) {
    ...
    router.get(route('admin.banners-accepts.index'), rest, { preserveState: true, replace: true });
}, 300));
</script>
