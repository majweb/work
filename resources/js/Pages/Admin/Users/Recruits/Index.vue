...
const impersonate = (id) => {
    router.post(route('admin.firms.impersonate', id));
};
...
const exportCsv = () => {
    ...
    window.location.href = route('admin.users.recruits.export') + '?' + queryString;
};
...
const updateFilters = () => {
    router.get(route('admin.users.recruits.index'), pickBy(filters.value), {
        preserveState: true,
        preserveScroll: true,
        replace: true
    });
...
const toggleBlock = () => {
    if (userToBlock.value) {
        router.post(route('admin.users.recruits.block', userToBlock.value.id), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
const verifyEmail = () => {
    if (userToVerify.value) {
        router.post(route('admin.users.recruits.verifyEmail', userToVerify.value.id), {}, {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <AppLayout title="Rekruterzy">
                ...
                <div class="bg-white rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-10 mb-8 transition-all hover:shadow-blue-900/10">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
                        <div class="flex items-center gap-6">
                            <div>
                                    <h3 class="text-2xl font-black text-[#0A2C5C] uppercase tracking-tight">Rekruterzy</h3>
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">Zarządzanie kontami rekruterów (recruit)</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-12">
                        <div class="bg-gray-50/50 rounded-[2rem] p-6 border border-gray-100 hover:border-blue-200 transition-all group">
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Suma rekruterów</p>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-50">
                                    ...
                                    <th class="p-8">
                                        <span class="text-[10px] font-black text-[#0A2C5C] uppercase tracking-[0.2em]">Firma</span>
                                    </th>
                                    <th class="p-8 text-center">
                                        <span class="text-[10px] font-black text-[#0A2C5C] uppercase tracking-[0.2em]">Projekty</span>
                                    </th>
                                    <th class="p-8 text-center">
                                        <span class="text-[10px] font-black text-[#0A2C5C] uppercase tracking-[0.2em]">Aplikacje</span>
                                    </th>
                                    <th class="p-8 text-center">
                            ...
                            <tbody>
                                <tr v-for="user in users.data" :key="user.id" class="group hover:bg-blue-50/30 transition-all duration-300 border-b border-gray-50 last:border-none" :class="{'bg-blue-50/50': selectedIds.includes(user.id)}">
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div class="flex items-center gap-2">
                                                    <Link :href="route('admin.users.recruits.edit', user.id)" class="text-xs font-black text-[#0A2C5C] group-hover:text-blue-600 transition-colors uppercase tracking-tight">
                                                        {{ user.name }}
                                                    </Link>
                                                    <span v-if="user.roles?.some(role => role.name === 'firm')" class="px-2 py-0.5 bg-blue-100 text-[#0A2C5C] text-[8px] font-black uppercase tracking-widest rounded-md border border-blue-200">
                                                        Główny
                                                    </span>
                                                </div>
                                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">{{ user.email }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-8 text-xs font-bold text-[#0A2C5C] uppercase tracking-tight">
                                        {{ user.user?.firm?.name || user.user?.name || '-' }}
                                    </td>
                                    <td class="p-8 text-center">
                                        <span class="inline-flex items-center justify-center px-3 py-1 bg-blue-50 text-[#0A2C5C] rounded-lg border border-blue-100 font-black text-xs">
                                            {{ user.projects_recruits_count || 0 }}
                                        </span>
                                    </td>
                                    <td class="p-8 text-center">
                                        <span class="inline-flex items-center justify-center px-3 py-1 bg-purple-50 text-purple-600 rounded-lg border border-purple-100 font-black text-xs">
                                        ...
                                        >
                                            ...
                                            >
                                                <Link
                                                    ...
                                                    :href="route('admin.users.recruits.edit', user.id)"
                                                ...
                                                <button
                                                    ...
                                                    class="w-full flex items-center gap-3 px-6 py-4 text-[10px] font-black uppercase tracking-widest text-orange-400 hover:bg-blue-50 transition-colors text-left group"
                                                >
                                                    <div class="w-8 h-8 rounded-xl bg-orange-50 flex items-center justify-center group-hover:bg-[#0A2C5C] transition-colors text-orange-400 group-hover:text-white">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4 transition-colors">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                                                        </svg>
                                                    </div>
                                ...
                                <tr v-if="users.data.length === 0">
                                    <td colspan="8" class="p-20 text-center">
                                        <div class="flex flex-col items-center gap-4 text-gray-300">
                                            ...
                                            <p class="text-xl font-black uppercase tracking-widest">Brak rekruterów</p>
                                            <p class="text-sm font-bold">Nie znaleziono rekruterów spełniających podane kryteria.</p>
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
        ...
        <DialogModal :show="userToBlock !== null" @close="userToBlock = null">
            ...
            <template #content>
                <p class="text-sm text-gray-600 font-bold">
                    Czy na pewno chcesz {{ userToBlock.user_blocked ? 'odblokować' : 'zablokować' }} konto rekrutera
                    <span class="text-[#0A2C5C]">{{ userToBlock.name }}</span> ({{ userToBlock.email }})?
                </p>
                <p v-if="!userToBlock.user_blocked" class="mt-2 text-xs text-red-500 font-medium">
                    Zablokowany rekruter nie będzie mógł zalogować się do systemu.
                </p>
            </template>
        ...
        <DialogModal :show="userToVerify !== null" @close="userToVerify = null">
            ...
            <template #content>
                <p class="text-sm text-gray-600 font-bold">
                    Czy na pewno chcesz ręcznie zweryfikować adres email rekrutera
                    <span class="text-[#0A2C5C]">{{ userToVerify.name }}</span> ({{ userToVerify.email }})?
                </p>
            </template>
