...
<template>
    <AppLayout :title="__('translate.social_media_title')">
            ...
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-white rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-10 mb-8">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-2xl font-black text-[#0A2C5C] uppercase tracking-tight">{{ __('translate.social_media_title') }}</h3>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">{{ __('translate.forInfoSocial') }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-12 items-center">
                        <div class="md:w-1/2">
                            <p class="text-sm font-bold text-gray-500 leading-relaxed uppercase">
                                {{ __('translate.social_media_description') }}
                            </p>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                    ...
                    <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100 flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/5 hover:-translate-y-1">
                        ...
                        <ul class="text-[10px] font-bold text-gray-400 space-y-3 uppercase tracking-widest list-none text-left w-full custom-list">
                            <li>{{ __('translate.social_media_promo_desc') }}</li>
                            <li>{{ __('translate.social_media_promo_target') }}</li>
                            <li>{{ __('translate.social_media_promo_detail') }}</li>
                        </ul>
                    </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100 flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/5 hover:-translate-y-1">
                        ...
                        <ul class="text-[10px] font-bold text-gray-400 space-y-3 uppercase tracking-widest list-none text-left w-full custom-list">
                            <li v-for="item in __('translate.social_media_include_content').split('\n')" :key="item">
                    ...
                    <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100 flex flex-col items-center transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/5 hover:-translate-y-1">
                        ...
                        <ul class="text-[10px] font-bold text-gray-400 space-y-3 uppercase tracking-widest list-none text-left w-full custom-list">
                            <li v-for="item in __('translate.social_media_use_content').split('\n')" :key="item">
                                <span>{{ item.replace('• ', '').replace('•', '') }}</span>
                            </li>
                        </ul>
                    </div>
                ...
                <div class="bg-[#0A2C5C] rounded-[3rem] p-12 text-center text-white mb-8 relative overflow-hidden shadow-2xl shadow-blue-900/20">
                    ...
                    <h2 class="text-2xl font-black mb-6 uppercase tracking-tight relative z-10">
                        {{ __('translate.social_media_action_title') }}
                    </h2>
                    ...
                    <div class="flex flex-wrap justify-center gap-8 mb-10 relative z-10">
                        ...
                        <div class="bg-white/10 backdrop-blur-sm px-8 py-4 rounded-3xl border border-white/10">
                            <p class="text-[10px] font-black uppercase tracking-widest text-blue-200 mb-1">
                                {{ __('translate.social_media_price') }}
                            </p>
                            <p class="text-2xl font-black">
                                {{ props.product?.price ? parseInt(props.product.price).toLocaleString() : '10 000' }} <span class="text-[10px] text-blue-200">{{ __('translate.points').toUpperCase() }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-if="props.check" class="mb-10 flex flex-wrap justify-center items-center gap-4 relative z-10">
                        <div class="text-[10px] font-black uppercase tracking-widest bg-white/5 py-4 px-8 rounded-2xl border border-white/5">
                            ...
                            {{ __('translate.social_media_active_to') }} <span class="text-blue-300 mx-2">{{ props.check.end.slice(0, 10) }}</span>
                        </div>
                    </div>

                    <div class="max-w-md mx-auto relative z-10">
                        <div class="mb-4 text-[10px] font-bold text-blue-200 uppercase tracking-widest">
                            {{ __('translate.admin_notification_info') }}
                        </div>
                        <button
                            v-if="firmPoints >= (props.product?.price || 10000)"
                            @click="handleExchange(props.product?.id || 11, props.product?.price || 10000)"
                        ...
                        >
                            {{ __('translate.social_media_button') }}
                        </button>
