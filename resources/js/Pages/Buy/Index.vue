        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                        <h2 class="text-blue-work text-right p-4">{{ __('translate.give') }}: <strong>{{points ? points : '0'}}</strong> {{ __('translate.points') }}</h2>
                    <div
                        class="p-6 lg:p-8 bg-white dark:bg-gray-800 dark:bg-gradient-to-bl dark:from-gray-700/50 dark:via-transparent border-b border-gray-200 dark:border-gray-700">
                        <h2 class="text-sm font-semibold text-blue-work">{{ __('translate.filters') }}</h2>
                        ...
                        <div class="col-span-6 flex mb-3 items-center">
                            <div class="flex mt-1 flex flex-col mr-2">
                                <InputLabel :value="__('translate.typeProduct')" class="text-sm"/>
                                ...
                                >
                                    ...
                                    <option value="points" id="points">{{ __('translate.points') }}</option>
                                    <option value="extra" id="extra">{{ __('translate.extra') }}</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div v-else>
                            <div class="grid grid-cols-1 lg:grid-cols-5 gap-2 lg:gap-8">
                                <div v-for="(product) in props.products.data" :key="product.id" class="mb-2 border border-2 border-gray-200 rounded-md p-5 flex flex-col items-center ">
                                    ...
                                    <p v-else class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ parseInt(product.price) }} {{__('translate.points')}}</p>
