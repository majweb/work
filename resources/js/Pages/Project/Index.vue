...
const params = ref({
    ...
    profession: null,
    positionSelect: null,
    external_company: props.filters.external_company
        ? props.externalCompanies.find(c => c.id == props.filters.external_company)
        : null,
});
...
watch(() => params.value.category, async (newCategory) => {
    ...
    try {
        const response = await fetch(route("projects.getChildsCategory", newCategory.value));
...
watch(() => params.value.categorySub, async (newCategorySub) => {
    ...
    try {
        const response = await fetch(route("projects.getChildsCategory", newCategorySub.value));
        const data = await response.json();
...
watch(() => params.value.profession, async (newProfession) => {
    ...
    try {
        const response = await fetch(route("projects.getChildsCategory", newProfession.value));
        const data = await response.json();
...
const applyFilters = () => {
    const transformedData = {
        ...
        external_company: params.value.external_company?.id || null
    };
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4 transition-all hover:shadow-blue-900/10 hover:-translate-y-1">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-black text-[#0A2C5C]">{{ stats.total_projects }}</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4 transition-all hover:shadow-blue-900/10 hover:-translate-y-1">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-black text-[#0A2C5C]">{{ stats.total_aplications }}</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4 transition-all hover:shadow-blue-900/10 hover:-translate-y-1">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-black text-[#0A2C5C]">{{ stats.active_projects }}</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-blue-900/5 border border-gray-100 flex items-center gap-4 transition-all hover:shadow-blue-900/10 hover:-translate-y-1">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-black text-[#0A2C5C]">{{ stats.inactive_projects }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <!-- Firma zewnętrzna -->
                        <div class="space-y-2">
                            <label class="block text-[10px] font-black text-gray-400 mb-2 uppercase tracking-widest">{{ __('translate.externalCompanyLabel') }}</label>
                            <multiselect
                                v-model="params.external_company"
                                :options="props.externalCompanies"
                                track-by="id"
                                label="name"
                                :selectLabel="''"
                                :selectedLabel="''"
                                :deselectLabel="''"
                                :placeholder="__('translate.externalCompanyLabel')"
                                class="custom-multiselect"
                            >
                                <template #noResult>
                                    <span>{{__('translate.noOptions')}}</span>
                        ...
                        <div class="flex flex-wrap items-center gap-6 mb-8">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex items-baseline gap-2">
                                    ...
                                    <span class="text-[10px] font-black text-blue-400 uppercase">{{ project.currency }}</span>
