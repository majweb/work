...
const form = ref({
    ...
    status: optionsStatus.value.find(o => o.value === props.filters?.status) || optionsStatus.value[0],
    ...
    has_candidate: optionsCandidateCreated.value.find(o => o.value === props.filters?.has_candidate) || optionsCandidateCreated.value[0],
...
watch(form, debounce(function (value) {
    let rest = pickBy({
        ...
        has_candidate: value.has_candidate?.value,
        date: value.date,
    });
                ...
                <div class="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-6 sm:p-10">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <multiselect v-model="form.status" :options="optionsStatus" label="name"
                                         track-by="value"
                                         :selectLabel="''"
                                         :selectGroupLabel="''"
                                         :selectedLabel="''"
                                         :deselectLabel="''"
                                         :placeholder="__('translate.applicationStatus')" class="custom-multiselect">
                                <template #noResult><span>{{ __('translate.noResults') }}</span></template>
                                <template #noOptions><span>{{ __('translate.noOptions') }}</span></template>
                            </multiselect>
                        </div>
                    ...
                    <div v-if="showAdditionalFilters">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <multiselect v-model="form.has_candidate" :options="optionsCandidateCreated" label="name"
                                             track-by="value"
                                             :selectLabel="''"
                                             :selectGroupLabel="''"
                                             :selectedLabel="''"
                                             :deselectLabel="''"
                                             :placeholder="__('translate.candidateCreated')" class="custom-multiselect">
                                    <template #noResult><span>{{ __('translate.noResults') }}</span></template>
                                    <template #noOptions><span>{{ __('translate.noOptions') }}</span></template>
                                </multiselect>
                            </div>
