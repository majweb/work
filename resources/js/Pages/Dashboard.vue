...
<template>
    <AppLayout :title="__('translate.dashboard')">
        ...
        <div class="py-12">
                ...
                <RecruitDashboard v-else-if="hasRole('recruit') && !hasRole('firm')" :adminBanner="adminBanner" :applications="otherAplications" :notifications="notifications" :projects="chartRecruit?.projects || []" :chartData="chartRecruit?.data || null" :lastAplications="chartRecruit?.applications" || []/>
                <FirmDashboard v-else-if="hasRole('firm')" :adminBanner="adminBanner" :firmData="chartFirm" :aplicationCount="aplicationCount" :viewCount="viewCount" :projectsCount="projectsCount" :recruitsCount="recruitsCount" :chartData="chartFirm?.recruits || null"  :chartDataApp="chartFirm?.aplications || null" :chartDataInvoices="chartFirm?.lastInvoices"  :notifications="notifications" :packages="packages" :additionalServices="additionalServices" :certificate="chartFirm?.certificate || null"/>
        </div>
    </AppLayout>
</template>
