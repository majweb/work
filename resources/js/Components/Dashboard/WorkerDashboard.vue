    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="p-4 sm:p-6 bg-white overflow-hidden shadow-xl sm:rounded-lg">
            ...
            <div class="space-y-8 sm:space-y-12">
                <!-- APLIKACJE -->
                <section class="bg-white rounded-[1.5rem] sm:rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 overflow-hidden relative">
                    <div class="absolute top-0 right-0 w-64 h-64 bg-blue-50/30 rounded-full -mr-32 -mt-32 blur-3xl opacity-50"></div>
                    <div class="p-4 sm:p-10 relative z-10">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-12">
                            <div>
                            ...
                            <Link :href="route('worker.aplications')"
                                  class="w-full sm:w-auto group flex items-center justify-center gap-2 px-6 py-3 bg-gray-50 text-[#0A2C5C] text-[10px] font-black rounded-xl sm:rounded-2xl hover:bg-blue-50 transition-all uppercase tracking-widest border border-gray-100">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12">
                            ...
                            <div class="lg:col-span-5 flex flex-col">
                                ...
                                <div @click="router.get(route('worker.singleAplication', props.lastAplications.id))" class="cursor-pointer flex flex-col sm:flex-row gap-6 sm:gap-10 items-center bg-gray-50/50 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] border border-gray-100/50 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-900/10 hover:bg-white group relative overflow-hidden">
                            ...
                            <div class="lg:col-span-7">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4" v-if="otherAplications.length">
                                    <div v-for="other in otherAplications" :key="other.id"
                                         ...
                                         class="cursor-pointer flex items-center gap-3 sm:gap-5 p-3 sm:p-5 rounded-[1.5rem] sm:rounded-[2.5rem] bg-gray-50/50 hover:bg-white transition-all duration-500 group border border-transparent hover:border-gray-100 hover:shadow-2xl hover:shadow-blue-900/10">
                                        <div class="relative shrink-0">
                                            <img :src="other.project?.user?.profile_photo_url"
                                                 class="w-10 h-10 sm:w-16 sm:h-16 rounded-[1rem] sm:rounded-[1.5rem] object-cover border-2 sm:border-4 border-white shadow-lg group-hover:scale-110 transition-all duration-500"
                                            />
                                            <div class="absolute -bottom-1 -right-1 w-3.5 h-3.5 sm:w-5 sm:h-5 bg-white rounded-full flex items-center justify-center shadow-md border border-gray-50">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 pb-12">
                    <!-- MOJE DANE -->
                    <section class="bg-white rounded-[1.5rem] sm:rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-4 sm:p-10 flex flex-col relative overflow-hidden group">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-12 relative z-10">
                            <div>
                            ...
                            <Link :href="route('profile.show')"
                                  class="w-full sm:w-auto group/edit flex items-center justify-center gap-2 px-6 py-3 bg-gray-50 text-[#0A2C5C] text-[10px] font-black rounded-xl sm:rounded-2xl hover:bg-blue-50 transition-all uppercase tracking-widest border border-gray-100">
                        ...
                        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-12 bg-gray-50/50 p-6 sm:p-10 rounded-[1.5rem] sm:rounded-[2.5rem] border border-gray-50/50 relative z-10 hover:bg-white transition-all duration-500 hover:shadow-xl hover:shadow-blue-900/5">
                            <div class="relative group/avatar">
                                <div class="w-24 h-24 sm:w-40 sm:h-40 rounded-[1.5rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white group-hover/avatar:scale-105 transition-transform duration-500">
                                    <img :src="auth.user?.profile_photo_url" class="w-full h-full object-cover" />
                                </div>
                                <div class="absolute -bottom-1 -right-1 sm:-bottom-2 sm:-right-2 bg-green-500 w-5 h-5 sm:w-8 sm:h-8 rounded-full border-2 sm:border-4 border-white shadow-lg shadow-green-200"></div>
                            </div>
                    ...
                    <!-- POWIADOMIENIA -->
                    <section class="bg-white rounded-[1.5rem] sm:rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 p-4 sm:p-10 flex flex-col relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-blue-50/30 rounded-full -mr-16 -mt-16 blur-2xl"></div>
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-12 relative z-10">
                            <div>
                            ...
                            <Link preserveScroll v-if="usePage().props.unreadNotifications" :href="route('notifications.markAllAsRead')" method="post" as="button"
                                  class="w-fit text-[10px] font-black text-[#e31e24] hover:text-red-700 transition-colors uppercase tracking-widest bg-red-50 sm:bg-transparent px-3 py-2 sm:p-0 rounded-lg sm:rounded-none">
