    ...
    public function share(Request $request): array
    {
        return array_merge(parent::share($request), [
            ...
            'registrationAgreements' => fn () => [
                'firm' => \App\Models\Agreement::where('type', 'firm_registration')->where('is_active', true)->whereNull('parent_id')->with('children')->get(['id', 'title', 'description', 'help_text', 'is_required']),
                'worker' => \App\Models\Agreement::where('type', 'worker_registration')->where('is_active', true)->whereNull('parent_id')->with('children')->get(['id', 'title', 'description', 'help_text', 'is_required']),
            ],
            'supportAgreements' => fn () => \App\Models\Agreement::where('type', 'tickets')->where('is_active', true)->whereNull('parent_id')->with('children')->get(['id', 'title', 'description', 'help_text', 'is_required']),
            ...
            'newsletterAgreements' => fn () => \App\Models\Agreement::where('type', 'newsletter')->where('is_active', true)->whereNull('parent_id')->with('children')->get(['id', 'title', 'description', 'help_text', 'is_required']),
