<?php


return [
    'meta' => [
        'title' => 'Work4you.global - Dodajte oglase za posao besplatno',
        'description' => 'Dodajte neograničen broj oglasa za posao i prikupljajte prijave kandidata potpuno besplatno. Preuzmite 20.000 bonus bodova za start!',
    ],
    'nav' => [
        'how_it_works' => 'Kako to radi',
        'benefits' => 'Prednosti',
        'faq' => 'FAQ',
        'register_free' => 'Otvorite besplatan račun',
    ],
    'hero' => [
        'title' => 'Dodajte neograničen broj oglasa za posao <br class="hidden lg:block"/> i prikupljajte prijave kandidata potpuno besplatno.',
        'subtitle' => 'Bez pretplate. Bez naknada za objavu. <br class="hidden md:block"/> Na startu odmah dobijate 20.000 bodova spremnih za korištenje.',
        'cta' => 'Otvorite besplatan račun i dodajte svoj prvi oglas',
        'target' => 'Idealno za regrutacijske agencije i <br class="hidden sm:block" /> kompanije koje zapošljavaju na veliko.',
    ],
    'model' => [
        'title' => 'Jednostavan model: Objavljujte besplatno i prikupljajte kandidate',
        'step1' => [
            'title' => 'Dodajete željeni broj oglasa',
            'desc' => 'Bez limita i bez ikakvih troškova',
        ],
        'step2' => [
            'title' => 'Primate prijave kandidata',
            'desc' => 'Prikupljate biografije (CV) kandidata potpuno besplatno',
        ],
        'step3' => [
            'title' => 'Koristite bodove samo kada vi to želite',
            'desc' => 'npr. za pristup CV-ovima ili za promociju oglasa',
        ],
    ],
    'credits' => [
        'badge' => 'za start!',
        'amount_label' => 'bodova',
        'test_platform' => 'Nakon registracije dobijate startni paket za potpuno testiranje platforme:',
        'benefits_list' => [
            'Pregled CV-ova kandidata (klasični, audio, video)',
            'Promocija oglasa za posao na vrhu liste',
            'Isticanje kompanije u katalogu poslodavaca',
            'Pristup naprednim filterima za regrutaciju',
        ],
        'no_start_costs' => 'Napokon regrutujete bez ikakvih početnih troškova...',
        'items' => [
            'free_ads' => 'Objavljujete oglase besplatno',
            'no_contracts' => 'Bez ugovora i mjesečnih pretplata',
            'own_rules' => 'Koristite platformu pod vlastitim uslovima',
            'cv_types' => 'Klasični, audio i video CV-ovi',
            'all_languages' => 'Oglasi na svim jezicima',
        ],
    ],
    'foundation' => [
        'badge' => 'Zajedno mijenjamo svijet:',
        'title' => '50% od iznosa svake kupovine finansira fondaciju koju vi izaberete.',
        'subtitle' => 'Zapošljavate radnike, a istovremeno pomažete onima kojima je to potrebno.',
    ],
    'cta' => [
        'title' => 'Spremni za besplatnu regrutaciju? <br/>Preuzmite svojih <span class="text-red-work">20.000 bodova</span> odmah!',
        'button' => 'Registrujte kompaniju besplatno',
    ],
    'who_is_it_for' => [
        'title' => 'Ko će imati najviše koristi?',
        'groups' => [
            'agencies' => 'Agencije za zapošljavanje i regruteri',
            'high_volume' => 'Kompanije s velikim obimom zapošljavanja',
            'cost_conscious' => 'Poslodavci koji žele smanjiti troškove',
            'international' => 'Kompanije koje posluju na međunarodnom nivou',
            'everyone' => 'Svako ko traži radnika',
        ],
    ],
    'faq_section' => [
        'title' => 'FAQ',
        'subtitle' => 'Često postavljana pitanja',
        'questions' => [
            [
                'question' => 'Da li je dodavanje oglasa zaista besplatno?',
                'answer' => 'Da — ne plaćate ništa za objavu, bez obzira na broj oglasa.',
            ],
            [
                'question' => 'Moram li kupiti bodove na samom početku?',
                'answer' => 'Ne — dobijate 20.000 bodova automatski odmah nakon registracije.',
            ],
            [
                'question' => 'Kada počinjem plaćati?',
                'answer' => 'Tek kada sami odlučite dokupiti dodatne bodove.',
            ],
            [
                'question' => 'Mogu li odustati u bilo kojem trenutku?',
                'answer' => 'Da — nema nikakvih ugovora niti obavezujućih rokova.',
            ],
        ],
    ],
    'footer' => [
        'description' => 'Prva regrutacijska platforma koja daje glas poslodavcima i kandidatima, dok istovremeno pruža stvarnu podršku humanitarnim fondacijama.',
        'nav' => 'Navigacija',
        'legal' => 'Pravne informacije',
        'privacy' => 'Politika privatnosti',
        'terms' => 'Uslovi korištenja',
        'rights' => 'Sva prava zadržana.',
    ],
    'sold' => [
        'hero' => [
            'title' => 'Vaši oglasi su privukli kandidate. <br/> Ne dozvolite im da čekaju.',
            'subtitle' => 'Povećajte broj prijava, otključajte pristup CV-ovima i iskoristite puni potencijal svojih oglasa.',
            'cta' => 'Dopunite račun',
            'warning' => 'Ne gubite kandidate koji se već prijavljuju!',
        ],
        'points_benefits' => [
            'title' => 'POGLEDAJTE ŠTA SVE MOŽETE UČINITI SA KUPLJENIM BODOVIMA',
            'items' => [
                'cv_access' => 'pristup CV-ovima kandidata',
                'cv_formats' => 'CV u različitim formatima',
                'ads_promo' => 'promocija oglasa za posao',
                'employer_distinction' => 'isticanje brenda poslodavca',
                'brand_building' => 'izgradnja jakog regrutacijskog brenda',
            ],
        ],
        'recharges' => [
            'title' => 'DOPUNA RAČUNA UMJESTO PRETPLATE',
            'subtitle' => 'Vaši bodovi, Vaša pravila...',
            'items' => [
                'free_ads' => [
                    'title' => 'Objavljujete oglase potpuno besplatno',
                    'desc' => 'Dodavanje oglasa uvijek ostaje besplatno. Gradite bazu kandidata bez ikakvih početnih troškova.',
                ],
                'no_expiration' => [
                    'title' => 'Vaši bodovi nikada ne ističu',
                    'desc' => 'Dopunjena sredstva su Vaša zauvijek. Možete ih iskoristiti danas, za sedmicu ili za mjesec dana za bilo koju aktivnost.',
                ],
                'pay_per_effect' => [
                    'title' => 'Plaćate samo za stvarni efekat',
                    'desc' => 'Mijenjate bodove samo kada želite iskoristiti konkretnu funkciju. Potpuna kontrola nad Vašim budžetom.',
                ],
            ],
        ],
        'foundation' => [
            'title' => 'Vaša odluka ima stvarnu moć:',
            'subtitle' => 'Polovica iznosa Vaše narudžbe bit će uplaćena direktno na račun izabrane fondacije.',
            'step1' => [
                'title' => 'Vi birate cilj:',
                'desc' => 'Vi odlučujete koju od organizacija ćemo podržati nakon što dopunite svoj račun.',
            ],
            'step2' => [
                'title' => 'Certifikat za Vašu kompaniju:',
                'desc' => 'Zvanična potvrda da uspješno povezujete proces zapošljavanja s društvenom odgovornošću.',
            ],
            'cta' => 'Izaberite paket bodova i navedite fondaciju',
        ],
        'solutions' => [
            'title' => 'Imamo rješenje za svakoga ko traži radnike:',
            'items' => [
                'agencies' => [
                    'title' => 'REGRUTACIJSKE AGENCIJE I REGRUTERI',
                    'desc' => 'Trenutni pristup klasičnim, audio ili video CV-ovima i napredni alati za selekciju omogućavaju Vam da završite proces prije konkurencije.',
                ],
                'high_volume' => [
                    'title' => 'KOMPANIJE S VELIKIM OBIMOM ZAPOŠLJAVANJA',
                    'desc' => 'Odsustvo fiksnih troškova donosi stvarnu uštedu. Bodovi se skidaju samo za konkretne akcije, dok su Vaši oglasi vidljivi stalno.',
                ],
                'cost_conscious' => [
                    'title' => 'POSLODAVCI KOJI ŽELE SMANJITI TROŠKOVE',
                    'desc' => 'Model bodovanja omogućava precizno upravljanje budžetom. Trošite tačno onoliko koliko ste planirali za određeno radno mjesto.',
                ],
                'international' => [
                    'title' => 'KOMPANIJE KOJE POSLUJU NA MEĐUNARODNOM NIVOU',
                    'desc' => 'Portal je dostupan na svim jezicima, što Vam omogućava da tražite radnike bilo gdje. Globalni domet i jednostavan način obračuna.',
                ],
                'everyone' => [
                    'title' => 'SVAKO KO TRAŽI RADNIKA',
                    'desc' => 'Bez obzira na veličinu Vašeg poslovanja, dobijate pristup visokomotivisanim kandidatima i istovremeno podržavate plemenit cilj.',
                ],
            ],
        ],
        'faq' => [
            'questions' => [
                [
                    'question' => 'Šta će se desiti sa mojim oglasima ako ne dopunim račun?',
                    'answer' => 'Oglasi ostaju aktivni. Dopuna računa je potrebna isključivo za otključavanje pregleda CV-ova kandidata, isticanje ponuda i druge promotivne opcije.',
                ],
                [
                    'question' => 'Koji je rok trajanja bodova?',
                    'answer' => 'Bodovi ne ističu. Možete ih iskoristiti sada ili prilikom narednih zapošljavanja za nekoliko mjeseci – sredstva Vas uvijek čekaju na računu.',
                ],
                [
                    'question' => 'Mogu li zamijeniti bodove za promociju oglasa?',
                    'answer' => 'Da. Bodovi su univerzalna valuta platforme – možete ih usmjeriti na otključavanje CV-ova, isticanje oglasa ili dodatnu promociju.',
                ],
                [
                    'question' => 'Kada ću dobiti pristup podacima kandidata?',
                    'answer' => 'Odmah nakon što zamijenite bodove. Dopunite račun bodovima, a zatim u panelu oglasa jednim klikom otključavate prijave.',
                ],
            ],
        ],
        'bottom_cta' => [
            'title' => 'Vaši kandidati čekaju na kontakt',
            'subtitle' => 'Dopunite račun za 30 sekundi i otključajte pune mogućnosti portala',
            'button' => 'Dopunite račun sada',
            'features' => [
                'free_ads' => 'besplatna objava oglasa',
                'no_expiration' => 'bezvremenski bodovi na Vašem računu',
                'real_support' => 'stvarna podrška onima kojima je potrebna',
            ],
        ],
    ],
    'charity' => [
        'nav' => [
            'how_it_works' => 'Kako to radi?',
            'benefits' => 'Prednosti',
            'faq' => 'FAQ',
            'register_foundation' => 'Registrujte fondaciju',
        ],
        'hero' => [
            'title' => 'Pretvorite procese regrutacije u <br class="hidden lg:block"/> stvarnu podršku za Vašu fondaciju.',
            'subtitle' => 'Pridružite se Ambasadorima portala. Svaka regrutacija je stvarna pomoć za Vaše štićenike. Bez troškova i suvišne birokratije.',
            'cta' => 'Registrujte fondaciju',
        ],
        'how_it_works' => [
            'title' => 'KAKO TO RADI?',
            'steps' => [
                'step1' => [
                    'title' => 'Kreirate profil',
                    'desc' => 'Kreirate vizit kartu Vaše organizacije koja je vidljiva poslodavcima na portalu, čime gradite prepoznatljivost Vaše misije.',
                ],
                'step2' => [
                    'title' => 'Vi ste Ambasador',
                    'desc' => 'Djelujete kao ambasador portala, pozivajući kompanije na društveno odgovorno zapošljavanje. Svaka njihova dopuna donosi 50% iznosa Vašoj fondaciji.',
                ],
                'step3' => [
                    'title' => 'Primate sredstva',
                    'desc' => 'Poslodavci sami biraju Vašu fondaciju kao cilj podrške ili dolaze do Vas putem unikatnog linka koji podijelite s njima.',
                ],
            ],
        ],
        'funds_origin' => [
            'title' => 'Odakle dolaze sredstva za fondacije?',
            'steps' => [
                'step1' => 'Poslodavac/kompanija provodi proces regrutacije',
                'step2' => 'Kupuje bodove potrebne za korištenje platforme',
                'step3' => '50% uplate ide na račun izabrane fondacije',
                'step4' => 'Fondacija uspješno realizuje svoju misiju',
            ],
        ],
        'why_model_works' => [
            'title' => 'ZAŠTO OVAJ MODEL RADI?',
            'items' => [
                [
                    'title' => 'Kompanije zapošljavaju svakog dana',
                    'desc' => 'Regrutacija je stalan i neophodan dio poslovanja hiljada kompanija i agencija za zapošljavanje.',
                ],
                [
                    'title' => 'Pomoć nastaje automatski',
                    'desc' => 'Podrška se generiše uporedo sa regrutacijom i ne zahtijeva nikakve dodatne donatorske uplate od biznisa.',
                ],
                [
                    'title' => 'Svi su na dobitku',
                    'desc' => 'Poslodavci zapošljavaju efikasnije, fondacije dobijaju sredstva, a kandidati pronalaze posao.',
                ],
                [
                    'title' => 'Svaka regrutacija pomaže',
                    'desc' => 'Svaka aktivnost na platformi direktno povećava obim podrške društvenim organizacijama.',
                ],
            ],
        ],
        'benefits' => [
            'title' => 'ZAŠTO VRIJEDI POSTATI NAŠ AMBASADOR?',
            'items' => [
                'new_source' => [
                    'title' => 'Novi izvor finansiranja',
                    'desc' => 'Dolazite do kompanija koje žele pomoći, ali traže jednostavan, transparentan i potpuno automatizovan način za realizaciju svojih CSR strategija.',
                ],
                'free_promo' => [
                    'title' => 'Besplatna promocija',
                    'desc' => 'Vaša fondacija je vidljiva svakom poslodavcu koji koristi portal, čime gradite prepoznatljivost unutar globalne poslovne zajednice.',
                ],
                'ready_tools' => [
                    'title' => 'Gotovi alati',
                    'desc' => 'Dobijate materijale koje možete slobodno dijeliti. Vi preporučujete efikasan alat za zapošljavanje, a kompanije koje traže radnike podržavaju Vaš cilj.',
                ],
                'business_relations' => [
                    'title' => 'Izgradnja odnosa s biznisom',
                    'desc' => 'Postajete partner kompanijama. Poslodavci dobijaju certifikat koji potvrđuje podršku Vašoj misiji, što dodatno učvršćuje vašu saradnju.',
                ],
            ],
        ],
        'business_card' => [
            'title' => 'Vaša vizit karta – Vaš prozor u svijet biznisa',
            'items' => [
                'describe' => 'Detaljno opišite čime se Vaša fondacija bavi i za šta su joj trenutno najpotrebnija sredstva.',
                'link' => 'Dodajte link do Vaše web stranice i kontakt podatke kako bi se svaka kompanija mogla bliže upoznati s Vašim radom.',
                'visible' => 'Vidljivi ste na listi izbora prilikom svake kupovine bodova, kao i u našoj sekciji „Partneri“.',
            ],
            'cta' => 'Registrujte svoju fondaciju sada',
        ],
        'how_to_increase_support' => [
            'title' => 'KAKO POVEĆATI PODRŠKU ZA SVOJU FONDACIJU?',
            'subtitle' => 'Nakon registracije dobićete vlastiti afiliacijski (partnerski) link.',
            'description' => 'Možete ga:',
            'items' => [
                'postaviti na Vašu web stranicu,',
                'dodati u potpis Vašeg e-maila,',
                'poslati poslovnim partnerima,',
                'dijeliti na društvenim mrežama,',
                'koristiti tokom fandrajzing akcija.',
            ],
            'footer' => 'Svaka kompanija koja iskoristi WORK4YOU.GLOBAL putem Vašeg linka može podržati upravo Vašu fondaciju.',
        ],
        'faq' => [
            'title' => 'FAQ',
            'questions' => [
                [
                    'question' => 'Koliko košta registracija?',
                    'answer' => 'Ništa. Registracija i održavanje profila fondacije su potpuno besplatni.',
                ],
                [
                    'question' => 'Kada dobijamo sredstva?',
                    'answer' => 'Sredstva se uplaćuju na račun fondacije u roku od 2-7 radnih dana nakon svake uplate koju izvrši poslodavac koji Vas podržava.',
                ],
                [
                    'question' => 'Moramo li promovisati portal?',
                    'answer' => 'To nije obaveza, ali biti Ambasador se isplati. Svaka kompanija koja zapošljava preko Vaše preporuke donosi garantovanih 50% vrijednosti svojih dopuna Vašoj organizaciji.',
                ],
                [
                    'question' => 'Koji dokumenti su potrebni?',
                    'answer' => 'Dovoljan je zvanični registracijski broj (npr. ID broj / KRS ili ekvivalent) i osnovni statutarni podaci koje ćemo verifikovati tokom kreiranja računa.',
                ],
                [
                    'question' => 'Odakle dolaze sredstva za fondaciju?',
                    'answer' => 'Sredstva dolaze od uplata poslodavaca koji koriste WORK4YOU.GLOBAL. Svaki put kada kompanija kupi bodove za korištenje platforme, 50% vrijednosti uplate ide izabranoj fondaciji.',
                ],
                [
                    'question' => 'Kako poslodavci biraju fondacije?',
                    'answer' => 'Poslodavci mogu samostalno izabrati fondaciju tokom korištenja platforme ili doći do nje putem individualnog linka koji dijeli sama fondacija.',
                ],
                [
                    'question' => 'Hoćemo li dobiti vlastiti profil na platformi?',
                    'answer' => 'Da. Svaka fondacija dobija vlastiti profil s opisom djelatnosti, kontakt podacima te mogućnošću prezentacije svoje misije i ciljeva.',
                ],
                [
                    'question' => 'Moram li promovisati portal na svojim društvenim mrežama?',
                    'answer' => 'Ne zahtijevamo redovnu promociju. Nakon pridruživanja programu, molimo Vas samo da objavite jedan informativni post o saradnji s WORK4YOU.GLOBAL. Dalja promocija je potpuno dobrovoljna.',
                ],
            ],
        ],
        'bottom_cta' => [
            'title' => 'Pridružite se krugu Partner Fondacija',
            'subtitle' => 'Registrujte se besplatno, kreirajte vizit kartu Vaše organizacije i počnite primati podršku od društveno odgovornih poslodavaca.',
            'cta' => 'Registrujte fondaciju',
        ],
        'our_mission' => [
            'title' => 'NAŠA MISIJA',
            'text1' => 'Društvena pomoć ne bi trebala zavisiti isključivo od povremenih humanitarnih akcija i prikupljanja donacija.',
            'text2' => 'Zato smo kreirali model u kojem kompanije mogu podržati fondacije kroz aktivnosti koje provode svakodnevno – zapošljavanje radnika.',
            'text3' => 'WORK4YOU.GLOBAL uspješno spaja svijet regrutacije sa svijetom društvene pomoći, stvarajući model koristan za sve strane.',
        ],
        'footer_features' => [
            'free_reg' => 'besplatna registracija',
            'simple_csr' => 'jednostavan CSR za biznis',
            'constant_support' => 'stalan izvor podrške',
        ],
    ],

];
